...
async function process_log(raw) {
  ...
  while (index < buffer.length) {
    ...
    if (converted) {
      let logDate = new Date(file_date.getTime() + converted.timestamp);
      if (!isNaN(logDate)) {
        converted.datetime = logDate.format('yyyy-mm-dd HH:MM:ss.l');
      } else {
        converted.datetime = "";  // 유효하지 않은 날짜는 공백으로 처리
        error++;
        console.error(`Invalid date: ${logDate}`);
      }
      log.push(converted);
    } else {
      error++;
...
var dateFormat = function () {
  ...
  return function (date, mask, utc) {
    ...
    if (isNaN(date)) throw SyntaxError('invalid date');//Console Error
...
Date.prototype.format = function(mask, utc) { return dateFormat(this, mask, utc); };//Console Error
